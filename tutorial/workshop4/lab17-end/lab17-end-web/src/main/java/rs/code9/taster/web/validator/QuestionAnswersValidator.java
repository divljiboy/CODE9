...
/**
 * Validates that there is minimum required number of answers and that at least one is correct.
 * 
 * @author p.stanic
 */
@Component
	...
	public void validate(Object target, Errors errors) {
		if (target != null && supports(target.getClass())) {
			...
			if (question.getAnswers().size() < MINIMUM_NR_OF_ANSWERS) {
				errors.rejectValue(ANSWERS_FIELD_NAME, MINIMUM_TWO_ANSWERS_MESSAGE_KEY);
			} else {
				int correctCount = 0;
				...
				if (correctCount < MINIMUM_NR_OF_CORRECT_ANSWERS) {
					errors.rejectValue(ANSWERS_FIELD_NAME, MINIMUM_ONE_CORRECT_ANSWER_MESSAGE_KEY);
				}
			}
		}
	}
